...
//@Override
- (int)intValue{
    JKBigInteger *intMax = [[JKBigInteger alloc] initWithString:@"2147483647"];//int max = pow(2, sizeof(int)*8 - 1) - 1 ;
    JKBigInteger *intMin = [[JKBigInteger alloc] initWithString:@"-2147483648"];//int min = -pow(2, sizeof(int)*8 - 1) ;
    int res = 0;
    if ([self compare:intMax]==NSOrderedAscending&&[self compare:intMin]==NSOrderedDescending) {
        res =  self.stringValue.intValue;
    }else{
        res = (int)[[self subtract:[[self shiftRight:32] multiply:[[JKBigInteger alloc] initWithString:@"4294967296"]]] stringValue].longLongValue;
    }
